...
power.d <- function(n, sigma.param, sigma.mult, 
  ...
  for (k in 1:length(B)){
    ...
    start <- Sys.time()
    out.row.col1 <- perm.test(n, d,gen.var, p,
    ...
    start <- Sys.time()
    out.row.col2 <- Multi.MMD(n, d,gen.var, p,
    ...
    out.row <- c(B[k],out.row.col1, out.row.col2, out.row.col1.time,
                 out.row.col2.time)
  ...
  colnames(out.compare) <- c("Resamples","Permutation-Gauss",
