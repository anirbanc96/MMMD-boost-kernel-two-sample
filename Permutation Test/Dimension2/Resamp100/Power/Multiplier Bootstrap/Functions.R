...
power.d <- function(n.seq, sigma.param, sigma.mult, 
  ...
  #        n.perm <- number of permutations done
  ...
  for (k in 1:length(n)){
    ...
    #--------------------------------------------------------------------------#
    #--------------------------------------------------------------------------#
    # Estimating power under multiplier bootstrap
    
    start <- Sys.time()
    out.row.col2 <- Multi.MMD(n[k], d,gen.var, p,
    ...
    out.row <- c(n[k],out.row.col2,out.row.col2.time)
  ...
  colnames(out.compare) <- c("Sample Size",
                             "MultiplierBoot-Gauss","Time-MultiplierBoot")
