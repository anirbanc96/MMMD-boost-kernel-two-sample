...
# n <- sample size
# x <- data
# k.vec <- list of kernels (kernlab object)
...
# kernel.choice <- choice of the kernel considered ("GAUSS" or "LAP")
...
# error.sigma <- vector of noise variance
...
power.d <- function(resamp, error.sigma,
  ...
  #----------------------------------------------------------------------------#
  out.compare <- foreach(k=1:length(error.sigma), .combine=rbind,
    ...
    n.y <- dim(data.Y)[1]
    #--------------------------------------------------------------------------#
    # adding gaussian noise to above data under H0 and H1
