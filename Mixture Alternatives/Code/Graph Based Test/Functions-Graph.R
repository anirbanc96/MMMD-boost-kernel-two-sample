...
power.d <- function(n, sigma.param, sigma.mult, 
  ...
  # redefining mixing probability vector for ease
  p <- p.seq
  ...
  out.compare <- foreach(k=1:length(p), .combine=rbind, .export = ls(envir=globalenv())) %dopar% {
    ...
    # Running the FR test
    out.row.col1 <- FR.test(n, d, p[k], test.type = "o", n.iter)
  ...
  colnames(out.compare) <- c("Mixing Probability", "FR test")
