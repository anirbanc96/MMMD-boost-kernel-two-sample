################################################################################
################# Driver Code for Power over Dimensions ########################
################################################################################
source("Functions.R")
#------------------------------------------------------------------------------#
#------------------------------------------------------------------------------#
start <- Sys.time()
# Number of repetitions
n.rep <- 50
# Number of data points
n <- 100
# Dimension vector of data
# Note: Change the following value for different dimensions
d <- 150
# probability of mixture
p <- seq(0,1,length.out = 6)
# parameter for sigma0 matrix generation
sigma.param <- 0.5
# parameter for sigma1 = c*sigma0 matrix generation
sigma.mult <- 1.25
# parameter for mean value for H1 distribution
mu.param <- 0

# Choice of kernel for single and multiple kernel tests
# poissble choices: 
# single - "GAUSS" or "LAP" for gaussian or laplacian
# multiple - "MINMAX", "GEXP" or "MIXED" for gaussian kernel with min-max, 
# exponential bandwidth choice, or mixture of gaussian and laplace kernel and
# "LAP" for laplace kernel with exponential bandwidth choice.

# 1st coordinate for single, 2nd for multiple
kernel.choice <- c("LAP","GAUSS", "LAP","GEXP", "MIXED")

# Libraries for parallelising
library(foreach)
library(doParallel)
library(snow)

cores <- strtoi(Sys.getenv("NSLOTS"))-1
cl <- makeCluster(cores, methods = FALSE, type = "MPI")

registerDoParallel(cl)

# Uncomment following lines and comment above if not using MPI
#cores <- detectCores()
#cl <- makeCluster(cores[1]-1) #not to overload your computer

#registerDoParallel(cl)


out.p <- c()
for (iter in 1:n.rep){
  # storing power values for particular iteration
  out.p.iter <- power.p(n, sigma.param = sigma.param, sigma.mult = sigma.mult,
                        mu.param = mu.param, d = d, p.seq = p,
                        kernel.choice = kernel.choice, n.iter = 500)
  out.p <- c(out.p, out.p.iter)
  print (iter)
}
out.p <- as.matrix(as.data.frame(out.p))
end <- Sys.time()
end-start

stopCluster(cl)

single.power.d1 <- 2+6*(0:(n.rep-1))
single.power.d2 <- 3+6*(0:(n.rep-1))
multi.power.d1 <- 4+6*(0:(n.rep-1))
multi.power.d2 <- 5+6*(0:(n.rep-1))
multi.power.d3 <- 6*(1:n.rep)
power.single.mat1 <- matrix(0, nrow = length(p), ncol = n.rep)
power.single.mat2 <- matrix(0, nrow = length(p), ncol = n.rep)
power.multi.mat1 <- matrix(0, nrow = length(p), ncol = n.rep)
power.multi.mat2 <- matrix(0, nrow = length(p), ncol = n.rep)
power.multi.mat3 <- matrix(0, nrow = length(p), ncol = n.rep)
for (k in 1:length(p)){
  power.single.mat1[k,] <- out.p[k,single.power.d1]
  power.single.mat2[k,] <- out.p[k,single.power.d2]
  power.multi.mat1[k,] <- out.p[k,multi.power.d1]
  power.multi.mat2[k,] <- out.p[k,multi.power.d2]
  power.multi.mat3[k,] <- out.p[k,multi.power.d3]
}

power.single.mat1 <- cbind(p,power.single.mat1)
power.single.mat2 <- cbind(p,power.single.mat2)
power.multi.mat1 <- cbind(p,power.multi.mat1)
power.multi.mat2 <- cbind(p,power.multi.mat2)
power.multi.mat3 <- cbind(p,power.multi.mat3)

write.csv(power.single.mat1, file = "SinglePower-LAP.csv")
write.csv(power.single.mat2, file = "SinglePower-GAUSS.csv")
write.csv(power.multi.mat1, file = "MultiPower-LAP.csv")
write.csv(power.multi.mat2, file = "MultiPower-GEXP.csv")
write.csv(power.multi.mat3, file = "MultiPower-MIXED.csv")
#------------------------------------------------------------------------------#
#------------------------------------------------------------------------------#