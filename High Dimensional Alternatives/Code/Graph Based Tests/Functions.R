...
# test.type <- type of test used. See gtests documentation for more details.
...
power.d <- function(n, sigma.param, sigma.mult, mu.param, d.seq, p, n.iter = 500){
  ...
  out.compare <- foreach(k=1:length(d), .combine=rbind, .export = ls(envir=globalenv())) %dopar% {
    ...
    # Notice that here we are dividing the mean vector by square root of
    # dimension which keeps the KL divergence between distributions
    # fixed
