...
Single.MMD <- function(n, d, gen.var, p = 0, kernel.choice,
                       n.iter = 1000){
...
Multi.MMD <- function(n, d,gen.var, p = 0, kernel.choice = "GEXP",
                      n.iter = 1000){
...
power.d <- function(n.seq, sigma.param = 0.4, sigma.mult = 1.1, 
  ...
  for (k in 1:length(n)){
    ...
    out.row.col5.time <- end-start
    #--------------------------------------------------------------------------#
    # Concatenating all the outputs
    out.row <- c(n[k],out.row.col1, out.row.col2, out.row.col3, out.row.col4,
                 out.row.col5, out.row.col1.time, out.row.col2.time,
                 out.row.col3.time, out.row.col4.time, out.row.col5.time)
  ...
  colnames(out.compare) <- c("Sample Size","Single Kernel-1","Single Kernel-2",
                             ...
                             "Multiple Kernel-3", "Time: Single Kernel-1",
                             "Time: Single Kernel-2",
                             "Time: Multiple Kernel-1",
                             "Time: Multiple Kernel-2",
                             "Time: Multiple Kernel-3")
