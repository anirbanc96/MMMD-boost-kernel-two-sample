...
# kernel.choice <- the kernel used ("GAUSS" or "LAP")
# Output
...
# sigma.mult.seq <- vector of the parameter for multiplying cov matrix under 
# H0 to get cov under H1
...
power.d <- function(n, sigma.mult.seq, mu.param, d, p,
                    kernel.choice, n.iter = 500){
  
  
  # redefining signal strength vector for ease
  sigma.mult <- sigma.mult.seq
  ...
  out.compare <- foreach(k=1:length(sigma.mult), .combine=rbind, .export = ls(envir=globalenv())) %dopar% {
    ...
    out.row.col1 <- Single.MMD(n, d, p,kernel.choice[1], n.iter)
    
    out.row.col2 <- Single.MMD(n, d, p,kernel.choice[2], n.iter)
    
    # Estimating power under multiple kernel test
    out.row.col3 <- Multi.MMD(n, d, p,kernel.choice[3], n.iter)
    
    out.row.col4 <- Multi.MMD(n, d, p,kernel.choice[4], n.iter)
    
    out.row.col5 <- Multi.MMD(n, d, p,kernel.choice[5], n.iter)
    
    # Concatenating all the outputs
    out.row <- c(sigma.mult[k],out.row.col1, out.row.col2, out.row.col3,
                 out.row.col4, out.row.col5)
  }
  
  out.compare <- as.data.frame(out.compare)
  colnames(out.compare) <- c("h","Single Kernel-1", "Single Kernel-2", 
