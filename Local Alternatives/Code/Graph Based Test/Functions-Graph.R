...
# test.type <- type of test used. See gtests documentation for details.
...
# sigma.mult.seq <- the vector of parameter for multiplying cov matrix under 
...
power.d <- function(n, sigma.mult.seq, 
  ...
  out.compare <- foreach(k=1:length(sigma.mult), .combine=rbind, .export = ls(envir=globalenv())) %dopar% {
    ...
    out.row <- c(sigma.mult[k],out.row.col1)
  }
  stopCluster(cl)
  
  out.compare <- as.data.frame(out.compare)
  colnames(out.compare) <- c("h", "FR test")
